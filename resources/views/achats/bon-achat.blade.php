...
<head>
    ...
    <title>Bon Achat — 7ssabHani</title>
    ...
    <style>
        ...
        .btn-pay {
            color: #1a1408;
            background: linear-gradient(135deg, #e2c45a, var(--gold));
        ...
        .data-table th {
            ...
            color: #f4f9ee;
            background: linear-gradient(145deg, #1c2a18 0%, #2e5c18 48%, #3f7a22 100%);
            border-bottom: none;
            white-space: nowrap;
        ...
        .modal {
            ...
            border-radius: 20px;
            box-shadow:
                0 24px 60px rgba(16, 24, 14, 0.35),
                0 0 0 1px rgba(139, 195, 74, 0.2),
                0 0 40px rgba(139, 195, 74, 0.12);
            overflow: hidden;
        ...
        .lines-table td {
            ...
            border-top: 1px solid rgba(21, 32, 20, 0.06);
            vertical-align: middle;
            text-align: center;
        }
        ...
        .btn-plus {
            ...
            background: linear-gradient(135deg, #b6e85f 0%, #8bc34a 45%, #6fad35 100%) !important;
            box-shadow: 0 0 14px rgba(139, 195, 74, 0.45), 0 4px 10px rgba(0, 0, 0, 0.12);
            display: grid;
            place-items: center;
            font-size: 1.35rem;
            font-weight: 700;
        ...
        .btn-add-line {
            ...
            font-size: 0.88rem;
            color: #102008;
            background: linear-gradient(135deg, #b6e85f, #8bc34a);
        ...
        .total-bar {
            ...
            padding: 0.85rem 1rem;
            border-radius: 12px;
            background: linear-gradient(135deg, rgba(139, 195, 74, 0.12), rgba(201, 162, 39, 0.1));
...
<body class="notranslate" translate="no">
    ...
    <div class="app">
        @include('partials.sidebar', ['activePage' => 'bon-achat', 'openMenu' => 'fournisseur'])
                ...
                <div class="table-card">
                    <div class="table-scroll">
                        <table class="data-table" id="bonsTable">
                            ...
                            <tbody id="bonsBody">
                                <tr class="empty-row">
                                    <td colspan="8" class="empty">Aucun bon d’achat — cliquez sur Ajouter</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <div class="modal-backdrop" id="modalBon" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
        <div class="modal">
            <div class="modal-head">
                <h2 id="modalTitle">Nouveau <span>Bon Achat</span></h2>
                ...
                <form id="formBon" autocomplete="off">
                    <div class="form-grid">
                        ...
                        <div class="field">
                            ...
                            <input type="text" id="fournisseur" name="fournisseur" placeholder="Nom du fournisseur" required>
                        </div>
                    </div>
    ...
    <div class="modal-backdrop" id="modalPay" role="dialog" aria-modal="true" aria-labelledby="modalPayTitle">
        <div class="modal" style="width:min(100%,720px)">
            <div class="modal-head">
                <h2 id="modalPayTitle">Paiement <span>Bon Achat</span></h2>
            ...
            <div class="modal-body">
                <div class="form-grid" style="grid-template-columns: repeat(3, minmax(0, 1fr));">
                    ...
                    <div class="field">
                        <label for="payFournisseur">Fournisseur</label>
                        <input type="text" id="payFournisseur" class="readonly" readonly tabindex="-1">
                    </div>
                    <div class="field">
    ...
    <script>
        ...
        function validateBonForm() {
            ...
            const frns = document.getElementById('fournisseur').value.trim();
            const lines = collectLines();
            const montant = updateTotal();

            if (!date || !frns) {
                alert('Veuillez renseigner Date et Fournisseur.');
                return null;
            }
            if (!lines.length) {
        ...
        function openPayModal() {
            ...
            document.getElementById('payFournisseur').value = data.frns;
        ...
        function ensureBonsEmptyState() {
            if (!bonsBody.querySelector('tr[data-id]')) {
                bonsBody.innerHTML = '<tr class="empty-row"><td colspan="8" class="empty">Aucun bon d’achat — cliquez sur Ajouter</td></tr>';
            }
        }
        ...
        function buildBonRow(bon) {
            ...
            tr.innerHTML =
                ...
                '<td>' + (window.AchatStore ? AchatStore.fmtMoney(bon.montant) : fmt(bon.montant) + ' DH') + '</td>' +
                '<td>' + (bon.typePaie || '') + '</td>' +
                '<td>' + (window.AchatStore ? AchatStore.fmtMoney(bon.montantPaye) : fmt(bon.montantPaye) + ' DH') + '</td>' +
                '<td class="col-solde">' + (window.AchatStore ? AchatStore.fmtMoney(bon.solde) : fmt(bon.solde) + ' DH') + '</td>' +
                '<td class="actions-cell">' + actionIcons() + '</td>';
            return tr;
        }

        function renderBons() {
            if (!window.AchatStore) {
                ensureBonsEmptyState();
                return;
            }
            const list = AchatStore.getBons();
            if (!list.length) {
                bonsBody.innerHTML = '<tr class="empty-row"><td colspan="8" class="empty">Aucun bon d’achat — cliquez sur Ajouter</td></tr>';
                return;
            }
        ...
        function initBonTableActions() {
            ...
            TableActions.setHandlers({
                ...
                delete: function (tr) {
                    ...
                    if (id && window.AchatStore) AchatStore.deleteBon(id);
        ...
        document.getElementById('btnPayValider').addEventListener('click', () => {
            ...
            if (window.AchatStore) {
                AchatStore.addReglementsFromPaiement({
                    date: data.date,
                    numBon: data.num,
                    fournisseur: data.frns,
                    lignes: lignes
