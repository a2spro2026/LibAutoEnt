...
<head>
    ...
    <title>Balance Achat — LibAutoEnt</title>
...
<body class="notranslate" translate="no">
    ...
    <div class="app">
        @include('partials.sidebar', ['activePage' => 'balance-achat', 'openMenu' => 'fournisseur'])
            ...
            <div class="page-wrap">
                <div class="analytics" aria-label="Cartes analytiques">
                    <div class="ana-card" data-tone="achats">
                        <div class="ana-label">Total Achats</div>
                    ...
                    <div class="table-scroll">
                        <table class="data-table" id="balanceTable">
                            ...
                            <tbody id="balanceBody">
                                <tr class="empty-row">
                                    <td colspan="6" class="empty">Aucune donnée — créez des bons d’achat pour afficher la balance</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <div class="modal-backdrop" id="modalDetail" role="dialog" aria-modal="true">
        <div class="modal">
            <div class="modal-head">
                <h2>Détail <span id="detailFrnsTitle">Fournisseur</span></h2>
    ...
    <script>
        ...
        const detailFrnsTitle = document.getElementById('detailFrnsTitle');
        let currentFournisseur = '';

        function updateCards() {
            if (!window.AchatStore) return;
            var t = AchatStore.getBalanceTotals();
            document.getElementById('cardTotalAchats').textContent = AchatStore.fmtMoney(t.montant);
            document.getElementById('cardTotalPaye').textContent = AchatStore.fmtMoney(t.montantPaye);
            document.getElementById('cardTotalSolde').textContent = AchatStore.fmtMoney(t.solde);
        }
        ...
        function renderBalance() {
            ...
            var rows = AchatStore.getBalanceFournisseurs();
            updateCards();

            if (!rows.length) {
                body.innerHTML = '<tr class="empty-row"><td colspan="6" class="empty">Aucune donnée — créez des bons d’achat pour afficher la balance</td></tr>';
                return;
            }

            body.innerHTML = rows.map(function (r) {
                return '' +
                    '<tr data-fournisseur="' + String(r.fournisseur).replace(/"/g, '&quot;') + '">' +
                    '<td>' + (r.date || '—') + '</td>' +
                    '<td>' + (r.fournisseur || '—') + '</td>' +
                    '<td>' + AchatStore.fmtMoney(r.montant) + '</td>' +
                    '<td>' + AchatStore.fmtMoney(r.montantPaye) + '</td>' +
                    '<td class="col-solde">' + AchatStore.fmtMoney(r.solde) + '</td>' +
        ...
        function openDetail(fournisseur) {
            if (!window.AchatStore || !fournisseur) return;
            currentFournisseur = fournisseur;
            detailFrnsTitle.textContent = fournisseur;

            var bons = AchatStore.getBonsByFournisseur(fournisseur);
            if (!bons.length) {
                detailBody.innerHTML = '<tr class="empty-row"><td colspan="5" class="empty">Aucun bon pour ce fournisseur</td></tr>';
            } else {
                detailBody.innerHTML = bons.map(function (b) {
                    return '' +
                        ...
                        '<td>' + (b.fournisseur || '—') + '</td>' +
                        '<td>' + (b.numero || '—') + '</td>' +
                        '<td>' + AchatStore.fmtMoney(b.montant) + '</td>' +
                        '<td>' + AchatStore.fmtMoney(b.montantPaye) + '</td>' +
                        '<td class="col-solde">' + AchatStore.fmtMoney(b.solde) + '</td>' +
                        '</tr>';
                }).join('');
            }
        ...
        function printDetail() {
            if (!currentFournisseur || !window.AchatStore) {
                window.print();
                return;
            }
            var bons = AchatStore.getBonsByFournisseur(currentFournisseur);
            var rows = bons.map(function (b) {
                return '<tr>' +
                    ...
                    '<td>' + AchatStore.fmtMoney(b.montant) + '</td>' +
                    '<td>' + AchatStore.fmtMoney(b.montantPaye) + '</td>' +
                    '<td>' + AchatStore.fmtMoney(b.solde) + '</td>' +
                    '</tr>';
            }).join('');
            ...
            w.document.write(
                '<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><title>Balance — ' + currentFournisseur + '</title>' +
                ...
                '<h1>Détail Balance Achat — ' + currentFournisseur + '</h1>' +
                '<table><thead><tr><th>Fournisseur</th><th>Bon N°</th><th>Montant Bon</th><th>Montant Payé</th><th>Solde</th></tr></thead>' +
        ...
        if (window.TableActions) {
            TableActions.setHandlers({
                view: function (tr) {
                    var frns = tr.getAttribute('data-fournisseur') || (tr.cells[1] ? tr.cells[1].textContent.trim() : '');
                    openDetail(frns);
                },
                print: function (tr) {
                    var frns = tr.getAttribute('data-fournisseur') || (tr.cells[1] ? tr.cells[1].textContent.trim() : '');
